...
public class UserInterface extends Application {
    ...
    private javafx.scene.control.ScrollPane menuScroller;
     ...
     */
    private void clearMenu(String screen, Runnable again) {
    ...
    private static void showIf(javafx.scene.Node node, boolean visible) {
        if (node == null) return;
     ...
     */
    private final java.util.Map<String, Double> innerScrollAt = new java.util.HashMap<>();
    ...
    private javafx.scene.control.ScrollPane keptScroller(String key, javafx.scene.Node content) {
        return remembering(innerScrollAt, key, content);
    }

    /** A scroller in one of the side panels, which always remembers. */
    private javafx.scene.control.ScrollPane keptPanelScroller(String key, javafx.scene.Node content) {
        return remembering(panelScrollAt, key, content);
    }
    ...
    private javafx.scene.control.ScrollPane remembering(
        ...
        scroller.vvalueProperty().addListener(
                (o, from, to) -> where.put(key, to.doubleValue()));
    ...
    private String cell(double value) {
     ...
     */
    private String shortTier(PayTier tier) {
        return switch (tier) {
     ...
     */
    private String flowText(double value) {
    ...
    private GamePrefs prefs = new GamePrefs();
     ...
     */
    private javafx.scene.layout.FlowPane buildStrip(String current) {
    ...
    private void showLandMenu() {
        clearMenu("showLandMenu", () -> showLandMenu());
        ...
        column.getChildren().add(statementLine("Mines standing on them",
                formatter.format(game.minesCommitted()),
                game.minesCommitted() < land.getIronDeposits() ? Palette.WARN : null));
    ...
    private static final double STATEMENT = 560;

    /** A statement's heading, with a rule under it. */
    private VBox statementHead(String title) { return statementHead(title, STATEMENT); }

    /** @param width for the one screen wider than a statement - the graph. */
    private VBox statementHead(String title, double width) {
     ...
     *             the news.
     */
    private HBox statementLine(String label, String value, String tone) {
    ...
    private Label statementNote(String text) {
    ...
    private VBox statementTotal(String label, String value, String tone) {
     ...
     */
    private StackPane parcelTile(LandParcel parcel, double going,
                                 boolean best, boolean richest) {

        boolean afford = parcel.getPrice() <= game.getCash();
    ...
    private void showPolicyMenu() {
        clearMenu("showPolicyMenu", () -> showPolicyMenu());
        ...
        rootMenu.getChildren().addAll(title, lead, policyVitals(), scrolled(column, 210));
    }

    /** Every tax line the city collected last month. */
    private double taxRaised() {
     ...
     */
    private java.util.List<String[]> policyFlags() {
        ...
        if (protectedCount > 0 && game.getTotalSubsidyPaid() > 0) {
            out.add(new String[] {Palette.WARN,
                    protectedCount + " sector" + (protectedCount == 1 ? " is" : "s are")
                            + " being kept alive",
                    money(game.getTotalSubsidyPaid()) + " last month. A protected sector "
        ...
        if (game.canReformCurrency()) {
            out.add(new String[] {Palette.ACCENT, "The money could be reformed",
                    ...
                    + "nothing else.", game.getPriceIndex().getIndex())});
        }
    ...
    private VBox flagLine(String tone, String heading, String body) {
    ...
    private HBox policyRow(String name, String blurb, String figure, String sub,
    ...
    private void drawPolicyScreen() {
        ...
        if (pages.length > 1) {
            ...
            rootMenu.getChildren().addAll(title, policyVitals(), strip,
                    scrolled(column, 250), back);
        } else {
            rootMenu.getChildren().addAll(title, policyVitals(),
                    scrolled(column, 210), back);
        }
    }
    ...
    private VBox leverHead(String figure, String what) {
     ...
     * cancel button.
     */
    private VBox stageSlider(String key, double current, double min, double max,
    ...
    private static double snapped(double value, double min, double step) {
        return step <= 0 ? value : min + Math.round((value - min) / step) * step;
    }

    private static boolean moved(double value, double from, double step) {
    ...
    private VBox wouldTotal(String label, String before, String after, String tone) {
        return statementTotal(label, before + "  →  " + after, tone);
    }

    /** Apply, or put it back. Nothing else on this bar. */
    private HBox applyBar(String label, Runnable apply) {
    ...
    private Label previewCaveat(String basis) {
    ...
    private static double scaled(double amount, double factor) {
     ...
     */
    private double wageTaxAtBase(TaxPolicy policy, double base) {
     ...
     */
    private double wageTaxWith(TaxPolicy policy, WageBand band, double base, double offset) {
        return wageTaxWith(policy, band, base, offset, null);
    }

    private double wageTaxWith(TaxPolicy policy, WageBand band, double base,
                               double offset, WageBand onlyBand) {
        double[] wages = game.getEconomyManager().getStaffedWagePerType();
        double tax = 0;
    ...
    private double payrollIn(WageBand band) {
        double[] wages = game.getEconomyManager().getStaffedWagePerType();
        double total = 0;
        for (int i = 0; i < wages.length && i < JobType.values().length; i++) {
    ...
    private void minimumWagePage(VBox column) {

        LabourMarket market = game.getLabourMarket();
        PopulationManager people = game.getPopulationManager();
        Education schools = game.getEducation();
        Healthcare hospitals = game.getHealthcare();
    ...
    private void policyRatePage(VBox column) {
    ...
    private void currencyReformPage(VBox column) {
        ...
        column.getChildren().add(statementLine("Prices since founding",
                String.format("%.2fx", game.getPriceIndex().getIndex()),
                Palette.TEXT_HEAD));
        ...
        column.getChildren().add(wouldBe("The city's cash",
                money(game.getCash()), money(game.getCash() / factor), Palette.TEXT_HEAD));
    ...
    private static String afterName(Denomination unit, double factor) {
    ...
    private void pensionPage(VBox column) {
        ...
        column.getChildren().add(stageSlider("contrib", contribution,
                0, TaxPolicy.MAX_CONTRIBUTION, .0025, UserInterface::pct2));
    ...
    private void tuitionPage(VBox column) {
    ...
    private static String burdenTone(double share) {
    ...
    private void subsidyPage(VBox column) {
        ...
        column.getChildren().add(statementTotal("Paid last month",
                signedTight(game.getTotalSubsidyPaid(), true),
                game.getTotalSubsidyPaid() > 0 ? Palette.WARN : Palette.TEXT_SPENT));
        column.getChildren().add(statementNote(
     ...
     */
    private javafx.scene.control.ScrollPane scrolled(VBox column, double chrome) {
        VBox content = new VBox(0);
        content.setAlignment(Pos.CENTER);
        column.setMaxWidth(javafx.scene.layout.Region.USE_PREF_SIZE);
        content.getChildren().add(column);

        javafx.scene.control.ScrollPane scroll = keptScroller(currentScreen + ":body", content);
    ...
    private static final String[] TRADE_MONEY_PAGES = {"The rate", "What is moving it"};
    private static final String[] TRADE_GOODS_PAGES = {"In and out", "Since founding"};

    private void showForeignMenu() {
        clearMenu("showForeignMenu", () -> showForeignMenu());
        ...
        rootMenu.getChildren().addAll(title, lead, tradeVitals(), scrolled(column, 210));
    }
     ...
     */
    private double ownReserves() {
        ForeignAccounts fx = game.getForeignAccounts();
        return fx.getReserves() - fx.getForeignDebt() - game.getCapitalFlows().getStock();
    }

    /** One subject on the trade landing. */
    private HBox tradeRow(String name, String blurb, String figure, String sub,
                          String tone, String area, String page) {
        ...
        row.setOnMouseClicked(e -> {
            tradeArea = area;
            tradePage = page;
            innerScrollAt.remove("showForeignMenu:body");
            showForeignMenu();
        });
        row.setOnMouseEntered(e -> row.setStyle(rest
                + Palette.block(Palette.RAISED, Palette.ACCENT)));
    ...
    private void drawTradeScreen() {
        ...
        rootMenu.getChildren().addAll(title, tradeVitals(), strip,
                scrolled(column, 250), back);
    }
    ...
    private record Flow(String name, String note, double amount, String colour) { }

    private void bopPicturePage(VBox column) {
     ...
     */
    private VBox bopRiver(java.util.List<Flow> in, java.util.List<Flow> out) {
     ...
     */
    private javafx.scene.shape.Path ribbon(double x0, double y0,
                                           double x1, double y1,
    ...
    private VBox bandLabel(Flow f, double x, double y, double width,
                           boolean rightAlign) {
    ...
    private void bopLedgerPage(VBox column) {
    ...
    private void reserveOwnPage(VBox column) {
     ...
     * seeing.
     */
    private VBox claimBar(double gross, double debt, double parked) {
    ...
    private void reserveCoverPage(VBox column) {
        ...
        if (hot.getStock() <= 0) {
            ...
            column.getChildren().add(statementLine("...as a share of its funding",
                    String.format("%.0f%%", game.getBank().hotFundingShare() * 100),
                    game.getBank().hotFundingShare() > .25 ? Palette.BAD : Palette.TEXT_MUTED));
     ...
     */
    private void exchangePage(VBox column) {
        ...
        double ceiling = tradeBuying ? Math.max(0, game.getCash()) : fx.sellableReserves();
        ...
        column.getChildren().add(statementLine("In the treasury",
                moneyFull(game.getCash()) + "  →  " + moneyFull(
                        tradeBuying ? game.getCash() - tradeExchange
                                    : game.getCash() + tradeExchange),
        ...
        confirm.setOnAction(e -> {
            if (tradeBuying) game.buyForeignCurrency(tradeExchange);
            else             game.sellForeignCurrency(tradeExchange);
            tradeExchange = 0;
            innerScrollAt.remove("showForeignMenu:body");
            showForeignMenu();
        });
    ...
    private boolean tradeBuying = true;
    private double tradeExchange = 0;

    /* =====================================================================
       THE CURRENCY
       ===================================================================== */

    private void currencyRatePage(VBox column) {
    ...
    private void currencyForcesPage(VBox column) {
     ...
     */
    private void forceLine(VBox column, String label, String value,
    ...
    private void tradeGoodsPage(VBox column) {
    ...
    private int priceRow(javafx.scene.layout.GridPane table, int line, String label,
    ...
    private void tradeRecordPage(VBox column) {
     ...
     */
    private String coverReading(double months) {
        if (months >= 120) return "over 10 years";
        return String.format("%.1f months", months);
    }

    private VBox coverMeter(ForeignAccounts fx) {
    ...
    private VBox backingMeter(ForeignAccounts fx, CapitalFlows hot) {
    ...
    private VBox parityMeter(ForeignAccounts fx) {
     ...
     */
    private VBox bandMeter(String label, String reading, double at,
    ...
    private static final String[] BANK_OWED_PAGES  = {"By borrower", "In trouble"};
    private static final String[] BANK_MONEY_PAGES = {"Deposits", "Funding"};
    private static final String[] BANK_BOOKS_PAGES = {"Income", "Balance sheet"};
    private static final String[] BANK_PAST_PAGES  = {"Lending", "Strain", "Capital"};

    private void showBankMenu() {
        clearMenu("showBankMenu", () -> showBankMenu());
        ...
        int months = game.getHistorySave().months();
        ...
        rootMenu.getChildren().addAll(title, lead, bankVitals(), scrolled(column, 210));
    }

    /** One subject on the bank's landing page. */
    private HBox bankRow(String name, String blurb, String figure, String sub,
                         String tone, String area, String page) {
    ...
    private void drawBankScreen() {
        ...
        rootMenu.getChildren().addAll(title, bankVitals(), strip,
                scrolled(column, 250), back);
    }
    ...
    private static final double GAUGE_MAX = 2.0;

    private void bankGaugePage(VBox column) {
     ...
     * to have gone round.
     */
    private StackPane strainGauge(double strain, double premium, double size) {
    ...
    private VBox gaugeKey(Bank bank) {
    ...
    private void bankLimitsPage(VBox column) {
    ...
    private int weightRow(javafx.scene.layout.GridPane table, int line,
     ...
     */
    private VBox limitBar(String name, String how, double limit,
    ...
    private void bankBranchPage(VBox column) {

        Bank bank = game.getBank();
        BuildingsTemplate branch = game.getBuildingManager()
    ...
    private void bankBorrowerPage(VBox column) {
    ...
    private void bankTroublePage(VBox column) {
    ...
    private void bankDepositPage(VBox column) {
    ...
    private void bankFundingPage(VBox column) {
    ...
    private void bankIncomePage(VBox column) {
    ...
    private void bankBalancePage(VBox column) {
    ...
    private void bankHistoryPage(VBox column, String page) {
    ...
    private void bankLendingHistory(VBox column, HistorySave h) {
    ...
    private void bankStrainHistory(VBox column, HistorySave h) {
    ...
    private void bankCapitalHistory(VBox column, HistorySave h) {
     ...
     * guard against it.
     */
    private VBox trendChart(String[] names, double[][] series, String[] colours) {
    ...
    private static double latest(double[] series) {
     ...
     */
    private String chartFigure(double value, String name) {
    ...
    private static final String[] BOOK_PAGES   = {"Every piece", "Buy back"};
    private static final String[] BORROW_PAGES = {"At home", "Abroad"};
     ...
     * happens once.
     */
    private record Instrument(String key, String name, String short_,
    ...
    private static Instrument instrument(String key) {
        for (Instrument i : INSTRUMENTS) if (i.key().equals(key)) return i;
        return INSTRUMENTS[2];
    }

    private static String instrumentColour(String type) {
        return switch (type) {
    ...
    private String borrowType = "Term";
    private int borrowTerm = 10;
    private double borrowAsk = 0;
    private boolean borrowHold = false;
    ...
    private void showFinanceMenu() {
        clearMenu("showFinanceMenu", () -> showFinanceMenu());
        ...
        double annual = annualGdp(game.getEconomyManager().getNationalAccounts());
        ...
        column.getChildren().add(financeRow("Borrow",
                ...
                String.format("%.2f%%", game.getInterestRate() * 100),
        ...
        rootMenu.getChildren().addAll(title, lead, financeVitals(), scrolled(column, 210));
    }
     ...
     */
    private HBox financeRow(String name, String blurb, String figure, String sub,
                            String tone, String area, String page) {
        ...
        row.setOnMouseClicked(e -> {
            financeArea = area;
            financePage = page;
            innerScrollAt.remove("showFinanceMenu:body");
            showFinanceMenu();
        });
        row.setOnMouseEntered(e -> row.setStyle(rest
                + Palette.block(Palette.RAISED, Palette.ACCENT)));
    ...
    private HBox financeVitals() {
        ...
        String moved = game.hasTreasuryMonth()
                ? (game.getTreasuryChange() >= 0 ? "+" : "−")
                        + money(Math.abs(game.getTreasuryChange())) + " last month"
                : "no month closed yet";
    ...
    private double couponNow(DebtManager owed) {
        double coupon = 0;
    ...
    private void drawFinanceScreen() {
        ...
        rootMenu.getChildren().addAll(title, financeVitals(), strip,
                scrolled(column, 250), back);
    }
    ...
    private void positionPage(VBox column) {
        ...
        column.getChildren().add(statementLine("Credit rating", game.getCreditRating()));
    ...
    private int ratioRow(javafx.scene.layout.GridPane table, int line,
    ...
    private static final int LADDER_YEARS = 12;

    private void ladderPage(VBox column) {

        java.util.List<Debt> paper = game.getDebtManager().getDebt();
        ...
        double revenue = game.getEconomyManager().getNationalAccounts().getTotalRevenue() * 12;
     ...
     */
    private double[][] ladderYears(java.util.List<Debt> paper, double[] extra) {
     ...
     */
    private VBox ladderChart(java.util.List<Debt> paper, double[] extra, String extraLabel) {
    ...
    private HBox keySwatch(String colour, String name) {
        Region dot = new Region();
        dot.setMinSize(10, 10);
        dot.setPrefSize(10, 10);
        dot.setMaxSize(10, 10);
    ...
    private void debtServicePage(VBox column) {
        ...
        double revenue = na.getTotalRevenue();
        double coupon = couponNow(ledger);
        double repaid = game.hasTreasuryMonth() ? game.getTreasuryRepaid() : 0;
     ...
     */
    private VBox serviceBands(double share) {
    ...
    private void homeAndAbroadPage(VBox column) {
    ...
    private void yourRatePage(VBox column) {
        ...
        double total = game.getInterestRate();
    ...
    private int rateRow(javafx.scene.layout.GridPane table, int line,
    ...
    private VBox stressRow(String label, double stress) {
    ...
    private void theBookPage(VBox column) {
    ...
    private void buyBackPage(VBox column) {
        ...
        column.getChildren().add(statementLine("Cash available",
                moneyFull(game.getCash()),
                game.getCash() > 0 ? Palette.GOOD : Palette.BAD));
    ...
    private VBox buyBackRow(Debt debt, double rate, int month) {

        double face = debt.getOustandingPrincipal();
        double price = debt.getMarketValue(rate);
        double gain = face - price;
        boolean affordable = price > 0 && price <= game.getCash();
    ...
    private void borrowPage(VBox column, boolean foreign) {
        ...
        /* ---------------------------- how much ---------------------------- */
        double minimum = game.minimumIssueSize();
        ...
        DebtQuote quote = foreign
                ? game.quoteForeign(kit.key(), borrowAsk, borrowTerm, kit.rounding())
                : game.quoteDebt(kit.key(), borrowAsk, borrowTerm, kit.rounding());
        ...
        confirm.setOnAction(e -> {
            String summary = foreign
                    ? game.handleForeignLogic(kit.key(), borrowAsk, borrowTerm,
     ...
     */
    private double[] proposedSchedule(String type, DebtQuote quote,
        ...
        if (foreign) {
            double rate = game.getDebtManager().getExchangeRate();
    ...
    private Label stepChip(String text, Runnable act, boolean quiet) {
        Label chip = new Label(text);
    ...
    private void showForeignDefaultMenu() {
        clearMenu("showForeignDefaultMenu", () -> showForeignDefaultMenu());
    ...
    private void handleAllBuildingMenus(String menuTitle, EnumSet<BuildingType> categories) {
        clearMenu("handleAllBuildingMenus", () -> handleAllBuildingMenus(menuTitle, categories));
    ...
    private static final double TILE_HEIGHT = 232;
    private static final double TILE_GAP = 10;
    ...
    private HBox constraintsBar() {
        ...
        double free = land.getAvailableSqFt();
        double used = land.getUtilisation();
        double materials = game.getConstructionMaterials();
    ...
    private final java.util.Set<String> pinnedStats = new java.util.HashSet<>();

    /** How many of each the player has dialled up, by building name. */
    private final java.util.Map<String, Integer> orderQty = new java.util.HashMap<>();

    private StackPane buildingTile(BuildingsTemplate template, String menuTitle,
        ...
        int owned = game.getBuildingManager().getQuantity(template.getId());
        ...
        Runnable reprice = () -> {
            ...
            } else if (q.total > game.getCash()) {
                tone = Palette.WARN;
                verdict = "short " + money(q.total - game.getCash())
    ...
    private Button stepper(String glyph, double width, Runnable go) {
     ...
     */
    private String runningCost(BuildingsTemplate t) {
     ...
     */
    private VBox tileStats(BuildingsTemplate t) {
    ...
    private HBox statPair(String label, String value) {
        Label what = new Label(label);
        what.setStyle(Palette.words(Palette.SIZE_CAPTION, Palette.TEXT_LABEL));
        Region gap = new Region();
        HBox.setHgrow(gap, Priority.ALWAYS);
     ...
     */
    private void placeOrder(BuildingsTemplate template, int quantity,
                            String menuTitle, EnumSet<BuildingType> categories) {

        Game.BuildResult result = game.buildStack(template, quantity, false);
    ...
    private HBox receiptCorner(String menuTitle, EnumSet<BuildingType> categories) {
        ...
        card.getChildren().addAll(head, what,
                ...
                receiptLine("Total cost", money(game.getTotalBuildingCost())));
    ...
    private void showNoDepositMenu(BuildingsTemplate selected, int quantity,
                                   String menuTitle, EnumSet<BuildingType> categories) {
        clearMenu("showNoDepositMenu", () -> showNoDepositMenu(selected, quantity, menuTitle, categories));
    ...
    clearMenu("showQuickDebtMenu", () -> showQuickDebtMenu(selected, quantity, prevTitle, prevCats));
    ...
    private void showFundingFellShortMenu(BuildingsTemplate selected, int quantity,
                                          String prevTitle, EnumSet<BuildingType> prevCats) {
        clearMenu("showFundingFellShortMenu", () -> showFundingFellShortMenu(selected, quantity, prevTitle, prevCats));
        ...
        Label what = new Label(String.format(
                ...
                money(price), money(game.getCash()),
                money(Math.max(0, price - game.getCash()))));
        what.setWrapText(true);
    ...
    private void showDebtResultMenu(String summary) {
        clearMenu("showDebtResultMenu", () -> showDebtResultMenu(summary));
    ...
    private void showSectorMenu() {
        clearMenu("showSectorMenu", () -> showSectorMenu());
        ...
        rootMenu.getChildren().addAll(title, lead, scrolled(column));
    }
    ...
    private static final double BOOK_NOW = 116;
    private static final double BOOK_THEN = 104;

    /** The column headings, once, at the top of a statement. */
    private HBox bookHead(String left) {
     ...
     */
    private HBox bookLine(String label, double now, double then, boolean known, String tone) {
    ...
    private void investorPage(VBox column, PolicySector sector,
        ...
        String said = game.getLastInvestment(key);
        ...
        if (game.getLandBlockedSectors().contains(key)) {
    ...
    private record ServiceArea(String name, String[] pages) { }

    private static ServiceArea[] serviceAreas() {
    ...
    private void showServicesStatsMenu() {
         ...
         */
        rootMenu.getChildren().addAll(title, areaVitals(), strips, scrolled(column, 250));
    }
    ...
    private void openPage() {
        innerScrollAt.remove(currentScreen + ":body");
    ...
    private static ServiceArea currentAreaFor(String name) {
        for (ServiceArea area : serviceAreas()) {
     ...
     */
    private javafx.scene.layout.FlowPane chipStrip(String[] names, String current,
    ...
    private HBox vitalsBar(VBox... cells) {
        if (cells.length > 0) {
    ...
    private HBox healthVitals() {
        ...
        return vitalsBar(
                limitCell("OFF SICK", String.format("%.1f%%", sick * 100),
                        people(game.getPopulationManager().getWorkforce() * sick)
                                + " of the workforce",
    ...
    private void healthPage(VBox column) {
        switch (servicePage) {
     ...
     */
    private void livingCarePage(VBox column, CareType care) {
     ...
     */
    private void deathCarePage(VBox column) {
     ...
     */
    private boolean educationNotRunYet() {
        double[] places = game.getBuildingManager()
                .getStaffedEducationPlaces(game.getPopulationManager().getJobFillRate());
        return sum(places) > 0 && game.getEducation().getGrossCost() <= 0;
    }

    private void educationPage(VBox column) {
     ...
     */
    private void basicLadderPage(VBox column) {
    ...
    private void coursePage(VBox column, EducationType course) {
    ...
    private void professionsPage(VBox column) {
     ...
     */
    private VBox courseBlock(EducationType course) {
    ...
    private VBox buildingItWouldNeed(EducationType course, LabourMarket market,
    ...
    private String returnNote(EducationType course, LabourMarket market, double back) {
        if (back <= 0) {
     ...
     */
    private VBox pipelineBars(double[] queue) {
    ...
    private HBox utilityVitals() {

        ServicesManager services = game.getServicesManager();
        ...
        return vitalsBar(
                ...
                limitCell("ROADS", String.format("%.0f%%", roads * 100),
                        game.getServicesManager().getInfrastructureManager().getStatus()
                                .toLowerCase(),
    ...
    private void utilityPage(VBox column) {
        switch (servicePage) {
    ...
    private void powerPage(VBox column) {
    ...
    private void waterPage(VBox column) {
    ...
    private void roadsPage(VBox column) {

        InfrastructureManager roads =
                game.getServicesManager().getInfrastructureManager();
     ...
     */
    private javafx.scene.layout.FlowPane utilityLinks(BuildingType type) {
    ...
    private HBox bookRow(String label, double thousands, String tone) {
        return statementLine(label, tightMoney(toDollars(thousands), false), tone);
    }

    /* --------------------------------- HEALTH --------------------------------- */

    private void healthBooksPage(VBox column) {
    ...
    private void educationBooksPage(VBox column) {
    ...
    private void utilityBooksPage(VBox column) {
        ...
        column.getChildren().add(statementTotal("Net income",
                tightMoney(toDollars(
                        game.getServicesManager().getServiceNetIncome()), false),
                game.getServicesManager().getServiceNetIncome() < 0
                        ? Palette.BAD : Palette.GOOD));
        column.getChildren().add(statementLine("Staff turnout",
                String.format("%.0f%%", uh.getAverageUtilityFill() * 100),
                uh.getAverageUtilityFill() < .9 ? Palette.WARN : null));
    ...
    private Button buildLink(String label, String category, EnumSet<BuildingType> types) {
        Button go = new Button(label + "  →");
        go.setStyle(Palette.words(Palette.SIZE_LABEL, "white")
                + " -fx-background-color: " + Palette.CONFIRM + ";");
        go.setOnAction(e -> handleAllBuildingMenus(category, types));
    ...
    private void showPopulationInfoMenu() {
        clearMenu("showPopulationInfoMenu", () -> showPopulationInfoMenu());
        ...
        if (anyLicence) {
            ...
            for (EducationType type : EducationType.values()) {
                ...
                if (n <= 0) continue;
                inDetail.getChildren().add(statementLine("  " + jobLabel(job),
        ...
        column.getChildren().add(statementLine("Homes built for",
                people(game.getHouseholdCapacity())));
        ...
        column.getChildren().add(ladderTable(pm, market, game.getEducation().getStudying()));
        column.getChildren().add(statementNote(
        ...
        rootMenu.getChildren().addAll(title, scrolled(all), cashflow);
    }
     ...
     */
    private VBox statementDisclosure(String label, String value, VBox detail) {
        return statementDisclosure(label, value, detail, "who?");
    }

    /** As above, naming what opening it shows. */
    private VBox statementDisclosure(String label, String value, VBox detail, String hint) {
    ...
    private Label sentence(String text, String tone) {
    ...
    private Label subHead(String text) {
     ...
     */
    private VBox alert(String heading, String body) {
    ...
    private static final double PYRAMID_BAR = 200;
     ...
     */
    private HBox pyramidRow(String label, double count, double share, boolean workingAge) {
    ...
    private javafx.scene.layout.GridPane grid(double[] widths, javafx.geometry.HPos[] align) {
    ...
    private void gridHead(javafx.scene.layout.GridPane table, String... names) {
    ...
    private static javafx.geometry.HPos[] rightAfterFirst(int columns) {
    ...
    private javafx.scene.layout.GridPane mixTable(double[] mix, double total) {
    ...
    private javafx.scene.layout.GridPane ladderTable(PopulationManager pm,
                                                     LabourMarket market,
                                                     double[] studying) {
    ...
    private javafx.scene.layout.GridPane professionTable(PopulationManager pm,
                                                         LabourMarket market) {
        boolean any = false;
        for (JobType job : JobType.values()) {
        ...
        for (EducationType type : EducationType.values()) {
            ...
            table.add(gridCell(jobLabel(job), tone, Palette.SIZE_CAPTION, false), 0, line);
    ...
    private javafx.scene.layout.GridPane jobTable(int[] jobs, int[] vacancies,
        ...
        for (int i = 0; i < jobs.length; i++) {
            ...
            table.add(gridCell(jobLabel(kinds[i]), tone, Palette.SIZE_CAPTION, false), 0, line);
    ...
    private String openCell = "";

    /** Set by a click that opens a cell, so the panel it opened is scrolled to. */
    private boolean revealOpened = false;

    /** What showSectorReport should bring into view on this draw, once. */
    private javafx.scene.Node revealTop = null;
    private javafx.scene.Node revealBottom = null;

    private void showHouseholdMenu() {
        clearMenu("showHouseholdMenu", () -> showHouseholdMenu());
        ...
        VBox all = new VBox(0, vitals, column);
        all.setAlignment(Pos.CENTER);
        showSectorReport("HOUSEHOLD CASH FLOW", all, this::showPopulationInfoMenu);
    }

    /** One of the two view buttons over the grid. */
    private Button viewTab(String label, boolean on, Runnable go) {
    ...
    private static final double TIER_COL = 88;
    private static final double SHAPE_COL = 168;
     ...
     */
    private VBox affordabilityGrid(HouseholdAccounts hh, HouseholdBalance bal,
     ...
     */
    private Label cashCell(HouseholdAccounts hh, FamilyModel families,
    ...
    private Label gridCell(String text, String tone, int size, boolean rightAlign) {
     ...
     */
    private HBox flowBar(double tax, double rent, double fees, double shops,
    ...
    private Region barPart(double width, String colour, String what) {
    ...
    private String govPage = "Overview";

    private static final String[] GOV_PAGES =
    ...
    private record Slice(String name, double amount, String colour) { }
     ...
     */
    private java.util.List<Slice> topSlices(java.util.List<String> names,
     ...
     */
    private StackPane donut(java.util.List<Slice> slices, String centreTop,
     ...
     */
    private VBox donutKey(java.util.List<Slice> slices, double total, double annualGdp) {
     ...
     */
    private VBox balanceBars(double revenue, double spending) {
    ...
    private HBox balanceBar(String label, double value, double scale, double width,
     ...
     */
    private VBox keyedBar(java.util.List<Slice> parts, double width) {
    ...
    private VBox stackedBar(java.util.List<Slice> parts, double width) {
    ...
    private void showGovernmentMenu() {
        clearMenu("showGovernmentMenu", () -> showGovernmentMenu());
        ...
        rootMenu.getChildren().addAll(title, vitals, strip, scrolled(column, 250));
    }
     ...
     */
    private double annualGdp(NationalAccounts na) {
    ...
    private boolean gdpEstimated(NationalAccounts na) {
        int months = na.getMonthsRecorded();
        return months > 0 && months < 12;
    }

    /** What a share of the year's output comes to, in words. */
    private String ofGdp(double monthly, double annualGdp) {
        if (annualGdp <= 0) return "no output to compare";
        return String.format("%.1f%% of annual GDP", monthly * 12 / annualGdp * 100);
    }

    private HBox governmentVitals(EconomyManager em, NationalAccounts na) {
    ...
    private void budgetOverview(VBox column, EconomyManager em, NationalAccounts na) {
        ...
        if (game.hasTreasuryMonth()) {
            ...
            double opening = game.getTreasuryOpening();
            double closing = game.getTreasuryClosing();
            double change  = game.getTreasuryChange();
            double raised  = game.getTreasuryRaised();
            double repaidP = game.getTreasuryRepaid();
            double booked  = game.getTreasurySurplus();
    ...
    private int bridgeRow(javafx.scene.layout.GridPane table, int line,
    ...
    private int shareRow(javafx.scene.layout.GridPane table, int line,
    ...
    private java.util.List<Double> revenueAmounts(EconomyManager em, NationalAccounts na) {
     ...
     */
    private VBox budgetLine(String label, double amount, double total, double annual,
    ...
    private HBox budgetHead(String left) {
        Label what = new Label(left);
        what.setStyle(Palette.words(Palette.SIZE_CAPTION, Palette.TEXT_LABEL));
        Region gap = new Region();
        HBox.setHgrow(gap, Priority.ALWAYS);
    ...
    private Label head(String text, double width) {
    ...
    private HBox payerRow(String who, double amount, double total, String rate) {
    ...
    private VBox salesTaxDetail(double total) {
        EconomyManager em = game.getEconomyManager();
        TaxPolicy policy = em.getTaxPolicy();
        VBox box = new VBox(0);
        for (PolicySector sector : PolicySector.values()) {
    ...
    private VBox wageTaxDetail(double total) {
        HouseholdAccounts hh = game.getHouseholds();
        VBox box = new VBox(0);
        for (int t = 0; t < HouseholdAccounts.RETIRED; t++) {
        ...
        box.getChildren().add(statementNote(String.format(
                ...
                + "policy screen.",
                game.getEconomyManager().getTaxPolicy().getIncomeTaxRate() * 100)));
        return box;
    }

    /** Pension contributions, by the tier that paid them. */
    private VBox contributionsDetail(double total) {
        HouseholdAccounts hh = game.getHouseholds();
        VBox box = new VBox(0);
        for (int t = 0; t < HouseholdAccounts.RETIRED; t++) {
    ...
    private VBox healthFeeDetail(double total) {
        Healthcare service = game.getHealthcare();
        VBox box = new VBox(0);
        for (CareType care : new CareType[]{CareType.GENERAL, CareType.CHILDCARE,
                                            CareType.SENIOR}) {
    ...
    private VBox schoolFeeDetail(double total) {
        Education schools = game.getEducation();
        double subsidy = schools.getTuitionSubsidy();
        VBox box = new VBox(0);
        for (EducationType course : EducationType.values()) {
    ...
    private void revenuePage(VBox column, EconomyManager em, NationalAccounts na) {
        ...
        column.getChildren().add(budgetHead(CityCalendar.format(game.getMonth())));
    ...
    private VBox revenueDetail(String name, double amount) {
        return switch (name) {
    ...
    private VBox healthSpendDetail(double total) {
        BuildingManager bm = game.getBuildingManager();
        double[] wages = game.getPopulationManager().getWagesPerType();
        ...
        box.getChildren().add(statementNote(String.format(
                ...
                tightMoney(toDollars(game.getHealthcare().getPayroll())),
                tightMoney(toDollars(game.getHealthcare().getUpkeep())))));
        return box;
    }

    /** Education spending, by the school it is spent on. */
    private VBox educationSpendDetail(double total) {
        BuildingManager bm = game.getBuildingManager();
        double[] wages = game.getPopulationManager().getWagesPerType();
        double[] fill = game.getPopulationManager().getJobFillRate();
        VBox box = new VBox(0);
        ...
        box.getChildren().add(payerRow("Tuition the city covers",
                game.getEducation().getSubsidy(), total, null));
        box.getChildren().add(statementNote(
    ...
    private record PaperKind(String name, int count, double principal,
                             double coupon, String note) { }

    private java.util.List<PaperKind> paperKinds() {
        ...
        for (Debt d : game.getDebtManager().getDebt()) {
    ...
    private PaperKind paperKind(String name) {
     ...
     *              share of it
     */
    private VBox debtServiceDetail(double total) {
        ...
        double repaid = game.getCityPrincipalRepaidThisMonth()
                + game.getForeignPrincipalRepaidThisMonth();
        if (repaid > 0) {
            box.getChildren().add(payerRow("Principal repaid \u2014 not an expense",
                    repaid, 0, "left the treasury"));
        }

        double foreignInterest = game.getForeignInterestPaidThisMonth();
    ...
    private VBox pensionDetail(double total) {
        EconomyManager em = game.getEconomyManager();
        HouseholdAccounts hh = game.getHouseholds();
        VBox box = new VBox(0);
    ...
    private void spendingPage(VBox column, EconomyManager em, NationalAccounts na) {
    ...
    private void outputPage(VBox column, EconomyManager em, NationalAccounts na) {
        ...
        graphs.setStyle(Palette.words(Palette.SIZE_LABEL, "white")
                + " -fx-background-color: " + Palette.ACCENT_FILL + ";");
        graphs.setOnAction(e -> showHistoryMenu());
    ...
    private static final double GRAPH = 760;

    /** A curated set of lines that belong on one chart. */
    private record Preset(String name, String blurb, String[] keys) { }
    ...
    private void showHistoryMenu() {
        ...
        if (h.months() < 2) {
            rootMenu.getChildren().addAll(title, lead, alert("Nothing to draw yet",
    ...
    private HBox historyVitals(HistorySave h) {
    ...
    private VBox historyRange(HistorySave h) {
    ...
    private Label pickChip(String text, boolean on, Runnable act) {
        Label chip = new Label(text);
        chip.setStyle("-fx-padding: 4 10 4 10; -fx-cursor: hand;"
                + " -fx-background-radius: " + Palette.RADIUS_TIGHT + ";"
                + " -fx-background-color: " + (on ? Palette.RAISED : Palette.CONTROL) + ";"
     ...
     */
    private VBox historyReading(HistorySave h, String key, String colour) {
     ...
     * meaningless.
     */
    private String changeText(String unit, double first, double last) {
    ...
    private static String marked(String prefix, String amount) {
     ...
     */
    private String signedTight(double thousands, boolean negate) {
        if (Math.abs(thousands) < .5) return "$0";
        boolean minus = negate != (thousands < 0);
        return (minus ? "\u2212" : "+") + money(Math.abs(thousands));
    }

    private String signed(double thousands, boolean negate) {
        if (Math.abs(thousands) < .5) return "$0";
        boolean minus = negate != (thousands < 0);
    ...
    private String usd(double thousands) {
        return marked(Currency.FOREIGN_SYMBOL, money(thousands));
    }

    /** ...and with every digit. */
    private String usdFull(double thousands) {
     ...
     */
    private String unitPrice(double thousands) {
    ...
    private Tab[] tabs() {
        return new Tab[] {
            new Tab("build",      Icons.BUILD,      "Build",              this::showBuildMenu),
             ...
             */
            new Tab("bank",       Icons.BANK,       "The bank",           this::showBankMenu),
            new Tab("trade",      Icons.TRADE,      "Trade & the world",  this::showForeignMenu),
            new Tab("policy",     Icons.POLICY,     "Policies",           this::showPolicyMenu),
            new Tab("reports",    Icons.REPORTS,    "Reports",            this::showHistoryMenu),
        };
    }
