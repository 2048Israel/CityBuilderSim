...
public class Bank {
    ...
    /* =====================================================================
       ...
       at RISK_BUSINESS; a business pays prime plus its own leverage and
       record (BusinessDebtManager.priceSector()), a household the four at
       RISK_HOUSEHOLD plus its months of income owed (Household.settle()),
       and the carry trade prime (lendingRate()). The city's own paper is NOT
     ...
     * in its spread - BusinessDebtManager's leverage and restructure
     * surcharge for a business, RISK_SLOPE on its months owed for a
     ...
     * a whole sector is one borrower, so the bank's own record is lumpy by
     * construction - a restructure writes a sector down to 60% of its assets,
     * and one sector can be half the book. Two versions were measured:
     ...
     * for the allowance the next batch gives it.
     */
     ...
     * batch 2's capital will; until 0.7.7 ratePremium() turned it into up to
    ...
    /* =====================================================================
       ...
       CAPITAL_RATIO, is the only number in them that is the city's:
         ...
         sound book its year's expected loss, BASE_LOSS_RATE of it ("stage
         1"); a book whose borrower is in trouble what the model would
         actually write off if it defaulted now, scaled by how close it is to
         that line (its lifetime loss, "stage 2"). A write-off is drawn
         ...
         shares back while it is at or over its target and issues new ones
         only while it is under it (buysBackOwnShares(), issuesOwnShares()),
         and those trades are capital, not income.
         Not capped at
    ...
    /** Leverage past which a business borrower is in trouble ("stage 2"): BusinessDebtManager.MAX_LOAN_TO_ASSETS, the most the shortfall desk lends against a borrower's assets - past it the bank would not lend it another dollar to cover a loss. */
    ...
    /** What a business owing this against these assets would cost the bank if it defaulted now: the restructure rule's write-down, everything owed past BusinessDebtManager.RESTRUCTURE_TARGET of its assets. */
    ...
    /**
     * The allowance a business's book holds: its year's expected loss while
     * the borrower is sound; once it is in trouble, lossIfDefaulted() scaled
     * from nothing at SECTOR_WATCH_LEVERAGE to all of it at the insolvency
     * line (BusinessDebtManager.INSOLVENCY_TRIGGER), where the restructure
     * would write exactly that off - and never less than the year's loss.
     */
    public static double sectorAllowance(double principal, double assets) {
        if (!(principal > 0)) return 0;
        double stage1 = principal * BASE_LOSS_RATE;
        if (!sectorWatched(principal, assets)) return stage1;
        double closeness = assets <= 0 ? 1
                : Math.min(1, (principal / assets - SECTOR_WATCH_LEVERAGE)
                        / (BusinessDebtManager.INSOLVENCY_TRIGGER - SECTOR_WATCH_LEVERAGE));
        return Math.max(stage1, lossIfDefaulted(principal, assets) * closeness);
    }
     ...
     *
     * @param sectors     each sector's name to {what it owes, its assets} - the
     *                    figures the restructure rule judges it on
    ...
    private void strikeAllowance(java.util.Map<String, double[]> sectors, double households, double householdsWatched) {
        ...
        if (sectors != null) {
            for (java.util.Map.Entry<String, double[]> e : sectors.entrySet()) {
                ...
                double held = sectorAllowance(v[0], v[1]);
    ...
    /** 2 when that sector's book is in trouble and holds its lifetime loss, 1 when it holds a year's. */
     ...
     * and a lock on the city's credit. A whole sector is one borrower in
     * this game, so a single restructure is a year of 20-100% of the book
     * (0.7.7's own finding, Bank.expectedLossRate()): seed 0 of the default
     ...
     * is one borrower, so the bank's largest default averaged 48-62% of its
     * weighted book and its target 51-65%; it lent freely at the ordinary
     * target while short of that, and the premium broke the borrowers it
     * was meant to slow. The batch's notes have the rest.
     */
     ...
     * own target (buysBackOwnShares()), and issues new ones to whoever buys
     ...
     * A stock and three things about the month, none of which the end of a
     * month can give back.
     */
    ...
    /** ...and back. Nothing on an older save, whose Income page reads zero for a month as it always did. */
