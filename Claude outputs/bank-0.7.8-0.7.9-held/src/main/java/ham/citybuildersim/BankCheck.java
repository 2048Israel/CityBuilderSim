...
public class BankCheck {
    ...
    public static void main(String[] args) throws Exception {
        ...
        close("a lightly leveraged business pays prime plus its own leverage and nothing else",
                written.getAnnualRate(), priced.prime(dial) + BusinessDebtManager.SPREAD_PER_DEBT_TO_ASSETS * .1, 1e-15);
        ...
        try {
            ...
            for (String key : new String[]{ "bankAllowance", "bankCapitalRecord", "bankMonthLines",
                    "bankStatementYear" }) json.remove(key);
        ...
        close("...its interest, its fees and its profit, as the Income page reads them",
        ...
        close("...by the same rule on the same borrowers as the bank that saved one, within a hundredth",
                opened.getAllowance(), livedBank.getAllowance(), .01 * livedBank.getAllowance());
    ...
    static void theBankAsABusinessWithItsCapital() {
        ...
        double lifetime = (40_000 - 30_000 * BusinessDebtManager.RESTRUCTURE_TARGET)
                * (lev - Bank.SECTOR_WATCH_LEVERAGE)
                / (BusinessDebtManager.INSOLVENCY_TRIGGER - Bank.SECTOR_WATCH_LEVERAGE);
        ...
        close("...and its allowance rises to what a default would cost, scaled by how near the insolvency line it is",
        ...
        // Month 3: its assets fall to 25,000 - past the line; the restructure writes it down.
        provider.startMonth();
        double setAside = provider.getSectorAllowance(IND);
        double loss = 40_000 - 25_000 * BusinessDebtManager.RESTRUCTURE_TARGET;
        provider.writeOffSector(IND, loss);
        double left = 40_000 - loss;
        provider.refresh(1, 1_000_000, 0, left, 0, 0);
        provider.provide(owing(IND, left, 25_000), 0, 0);
        out.printf("   written down by %,.1f with %,.1f set aside: provision %,.1f%n", loss, setAside, provider.provisions());
        close("a write-off is drawn against the allowance its book held first", provider.getAllowanceUsed(), setAside, 1e-9);
        close("...and only what that did not cover reaches the month", provider.getWriteOffsBeyondAllowance(), loss - setAside, 1e-9);
        ...
        assertTrue("...and the written-down borrower is back in stage 1", provider.getStage(IND) == 1);
        worstMove = Math.max(worstMove, Math.abs(provider.equity() - provider.getOpeningEquity() - provider.getNetIncome()));
        provided += provider.provisions(); writtenOff += provider.getWriteOffs();
        provider.closeMonth();
        ...
        // ...and a restructure still writes an insolvent borrower down.
        held.setAssets(IND, 10_000);
        assertTrue("...and an insolvent borrower is still restructured", held.restructureInsolventSectors() > 0);
