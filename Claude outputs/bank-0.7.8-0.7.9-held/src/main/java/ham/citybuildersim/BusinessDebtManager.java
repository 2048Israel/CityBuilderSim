 ...
 * PRICING (0.7.7)
 *
 *     rate = the bank's prime + this borrower's spread
 *
 ...
 * The spread is THIS borrower's: driven by leverage - debt to total assets -
 * which mirrors how DebtManager prices the city off debt-to-GDP, and by its
 * record of write-downs.
 *
 *     spread = MIN_SPREAD + SPREAD_PER_DEBT_TO_ASSETS * (debt / assets),
 *              clamped to [MIN_SPREAD, MAX_SPREAD], LESS MIN_SPREAD
 *
 * RE-BASED ON PRIME: MIN_SPREAD was the point a debt-free business paid over
 * the city "because it is still not the city", and prime already carries what
 * that point stood for - the bank's costs and the base expected loss - so it
 * comes off, and a sound, lightly leveraged business pays about prime. A
 * business whose debts exceed its assets pays MAX_SPREAD - MIN_SPREAD over
 * prime and no more - the cap is what stops a bad month from compounding
 * into an unrecoverable one.
 *
 * Capping the SPREAD rather than the total rate keeps the two systems coupled:
 * if the policy rate drives prime up, business credit follows it up rather
 * than compressing to nothing against a fixed ceiling. A loan keeps the rate
 * it was written at for its LOAN_TERM_MONTHS.
 *
 * ORIGINATION
 *
...
public class BusinessDebtManager {
    ...
    /** The floor of the leverage spread as it was struck over the city's rate: a debt-free business's point over sovereign. Taken off again since 0.7.7, because prime carries what it stood for - see PRICING. */
    static final double MIN_SPREAD = .01;

    /** The ceiling of the leverage spread, struck the same way - the "even worst case, not too bad" cap; a borrower pays at most MAX_SPREAD - MIN_SPREAD over prime. */
    static final double MAX_SPREAD = .08;
    ...
    /** What a restructured borrower is left owing, as a multiple of its assets. Public since 0.7.8: the bank's allowance reads what a default would cost it off this (Bank.lossIfDefaulted()). */
    ...
    static final double SPREAD_PER_DEBT_TO_ASSETS = .06;
    ...
    private double priceSector(String sector, double extraPrincipal) {
        ...
        /*
         * THIS BORROWER'S OWN RISK, and only its own: the leverage spread as
         * it was struck over the city's rate, less MIN_SPREAD, because prime
         * already carries the bank's costs and the base expected loss - see
         * PRICING. Counting that point again would price the same risk twice.
         */
        if (totalAssets <= 0) {
        ...
        spread = Math.max(MIN_SPREAD, Math.min(spread, MAX_SPREAD)) - MIN_SPREAD;
    ...
    /** What this sector pays over prime: its own leverage and record. */
    ...
    /**
     * Writes a loan of this principal. The borrower is owed the principal
     * LESS its fee - feeOn(faceValue), which the caller keeps back from what
     * it hands the sector and the bank collects at the settle.
     */
     ...
     * check). The bank's allowance reads a sector's position as that, with
     * whatever it has borrowed since on both sides (Game.sectorPositions()).
