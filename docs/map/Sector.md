# Sector.java - 1,784 lines · 149 methods · 7 constants · model

`ham/citybuildersim/Sector.java` - generated 2026-09-23 by CodeMap; line numbers are as of that run.
...
| 715 | THE LEDGER |
| 933 | THE STATEMENT |
| 1082 | THE MONTH AT THE BOTTOM - HOOKS Markets CALLS |
| 1153 | WHOSE COST IS IT, WHEN ONE LINE MAKES TWO THINGS |
| 1403 | PLANNING - the decision to grow, and to shrink |
| 1493 | THE SCREENS |
| 1650 | SAVE AND RESTORE |

## Enum constants

| line | constant | says |
|---:|---|---|
| 1503 | `Sector.Line.Kind.HEAD` |  |
| 1503 | `Sector.Line.Kind.LINE` |  |
| 1503 | `Sector.Line.Kind.NOTE` |  |
| 1504 | `Sector.Line.Tone.NONE` |  |
| 1504 | `Sector.Line.Tone.GOOD` |  |
| 1504 | `Sector.Line.Tone.WARN` |  |
| 1504 | `Sector.Line.Tone.BAD` |  |
| 1504 | `Sector.Line.Tone.MUTED` |  |
| 1504 | `Sector.Line.Tone.HEAD` |  |
...
| 794 | `public double salesToHouseholds` | Sold to households in particular - consumption, in the national accounts. |
| 841 | `private Ledger pending` |  |
| 942 | `public double revenue, inputs, payroll, electricity, water, maintenance` |  |
| 943 | `public double operatingIncome, interest, propertyTax, salesTax, preTaxIncome, profitTax, netIncome` |  |
| 945 | `public double localSales, exports, otherRevenue, salesToHouseholds` | The two halves of revenue, for the accounts. |
| 947 | `public double localPurchases, imports` | The two halves of inputs, for the accounts and the audit. |
| 948 | `public Map<String, Double> purchasesBySupplier` |  |
| 955 | `public Map<Good, Split> sold` | Revenue and the cost of sales BY GOOD, each split home and abroad - what the income statement's two biggest lines open into. |
| 956 | `public Map<Good, Split> bought` |  |
| 963 | `public Map<String, Double> otherParts` | And the parts of otherRevenue that have names - the builders' work recognised and repairs billed. |
| 969 | `public Map<String, Double> otherInputs` | ...and the parts of the INPUT line that are not a good: haulage on the shippers' books, fuel on the railway's. |
| 985 | `private final Statement statement` |  |
| 1267 | `protected final Map<Good, Double> pantryUsedLastMonth` |  |

## Methods, in file order, under their sections

| line | len | member | says |
|---:|---:|---|---|
| 73 | 1712 | **type** `public abstract class Sector` | One business in the city, and the template every sector extends. |
...
| 748 | 80 | **type** `public static final class Ledger` |  |
| 791 | 1 | `public Split soldOf(Good g)` _(in Sector.Ledger)_ |  |
| 792 | 1 | `public Split boughtOf(Good g)` _(in Sector.Ledger)_ |  |
| 796 | 5 | `public double purchases()` _(in Sector.Ledger)_ |  |
| 802 | 1 | `public double revenue()` _(in Sector.Ledger)_ |  |
| 804 | 9 | `void clear()` _(in Sector.Ledger)_ |  |
| 820 | 7 | `void scale(double s)` _(in Sector.Ledger)_ | AND THE PER-GOOD MONEY SCALES WITH EVERYTHING ELSE. |
| 830 | 10 | `public static Map<Good, Split> copyOf(Map<Good, Split> from)` | A deep copy, because a Split is mutable and the ledger is cleared under it. |
| 843 | 1 | `public Ledger pending()` |  |
| 846 | 14 | `protected void bookSale(Trade t)` | A sale of this sector's, booked. |
| 862 | 11 | `protected void bookPurchase(Trade t)` | A purchase of this sector's, booked. |
| 875 | 3 | `protected final void bookOtherRevenue(double amount)` | Revenue that is not a sale of a good, booked into the month. |
| 903 | 5 | `public final void billForService(String supplier, String what, double amount)` | A SERVICE BOUGHT FROM ANOTHER BUSINESS IN THE CITY, billed by the business that performed it. |
| 927 | 5 | `protected final void bookImportedService(String what, double amount)` | ...and one bought from the WORLD: an import with no good behind it. |

### THE STATEMENT (lines 933-1081)

| line | len | member | says |
|---:|---:|---|---|
| 941 | 43 | **type** `public static final class Statement` | The month's figures, as the statement was struck. |
| 971 | 12 | `void scale(double s)` _(in Sector.Statement)_ |  |
| 987 | 1 | `public Statement statement()` |  |
| 990 | 1 | `public double getNetIncome()` | Pre-tax, before the profit tax. |
| 993 | 1 | `public double getProfitTax()` | What the city collects from it this month. |
| 1000 | 28 | `public void strike()` | Strikes the month WITHOUT the sales tax and without banking: the ledger is read into the statement, and the VAT is struck from these figures next, by SalesTaxLedger, and cannot be known while they are written. |
| 1037 | 16 | `public void bank(double salesTaxRemitted)` | ...and banks it, once the sales tax is known. |
| 1055 | 1 | `protected void afterBank()` | A sector with something to clear when its month is banked says so here. |
| 1061 | 20 | `public void restoreStatement(Statement saved)` | The struck month, put back on load, so the first month back reads the same as the one before it and the loss counter sees what it saw. |

### THE MONTH AT THE BOTTOM - HOOKS Markets CALLS (lines 1082-1152)

| line | len | member | says |
|---:|---:|---|---|
| 1109 | 4 | `protected double plannedDemand(Good g)` | What the city will want of a good this month, for planning output. |
| 1120 | 9 | `public double getPlannedOutput(Good g)` | What it will make of a stockable good this month for the home market: nameplate at today's rate, or what brings the stock to STOCK_MONTHS of demand, whichever is less. |
| 1135 | 6 | `public double getExportBoundOutput(Good g)` | Nameplate the city cannot eat, made for export instead - if the export price clears the marginal cost of running the line, which is the energy and water and nothing else: the staff are paid either way. |
| 1146 | 6 | `public double getCostPerUnit(Good g)` | Break-even per unit this month: everything the line costs over what it makes. |

### WHOSE COST IS IT, WHEN ONE LINE MAKES TWO THINGS (lines 1153-1402)

| line | len | member | says |
|---:|---:|---|---|
| 1186 | 19 | `protected double costShareOf(Good g)` |  |
| 1213 | 6 | `public double getMarginalCostPerUnit(Good g)` | What it costs to SELL a unit already made: the energy and water, and the inputs, and not the payroll, which is paid whether or not a unit leaves the shed. |
| 1221 | 8 | `protected double inputCostAtRate()` | What the month's inputs cost at the operating rate, at today's prices. |
| 1237 | 19 | `public double bid(Good g)` | What it wants of an input this month. |
| 1258 | 5 | `protected double pantryTarget(Good g)` | Units of a pantry good the sector aims to hold: months of recent use. |
| 1265 | 1 | `protected double recentUse(Good g)` | What it used of a pantry good last month. |
| 1277 | 19 | `public double offer(Good g, double price)` | Units it will release to the market at the price. |
| 1302 | 10 | `public void produceFlow(Good g)` | Lifts, brews or smelts a flow good for the month - what the makers bring to a market that has no stock behind it. |
| 1314 | 1 | `protected double groundLimit(Good g, double asked)` | A sector whose output is limited by what is in the ground says so here. |
| 1320 | 12 | `public void shipUnsoldFlow(Good g, GoodsMarket market)` | A flow good's unsold units, once the market has taken what it wants: shipped abroad at the export price, or lost. |
| 1347 | 23 | `public void produceStock(Good g, GoodsMarket market)` | Runs the month's production of a stockable good into the warehouse, after the market has taken what it wanted from last month's stock. |
| 1372 | 4 | `void takeFromStock(Good g, double units)` | Units of a good taken out of stock by a local sale or an export from the shed. |
| 1378 | 5 | `void receiveInput(Good g, double units)` | Units of an input received into the pantry, or consumed on the spot. |
| 1385 | 6 | `protected final void usePantry(Good g, double units)` | A pantry good used up this month, recorded for next month's cover. |
| 1398 | 1 | `public void sellOwnPriced(Markets markets, Game game)` | The seller-priced goods, sold: the shops to the households, the landlords' doors to the families. |
| 1401 | 1 | `public void endOfMonth(Game game)` | After every market has cleared and every good is made: anything the month still needs. |

### PLANNING - the decision to grow, and to shrink (lines 1403-1492)

| line | len | member | says |
|---:|---:|---|---|
| 1420 | 3 | `public BusinessInvestment.Decision plan(BusinessInvestment plans, Game game)` | What this sector would like to build this month, or why not. |
| 1428 | 5 | `public Good planningGood()` | The good the generic planner sizes the sector by: its first stockable output, else its first output. |
| 1441 | 1 | `public double firstPlantUtilisation()` | The share of a plant's nameplate the city has to be taking, a month, before this sector sinks its FIRST plant. |
| 1450 | 1 | `public double visibleDemandOver(double months, EconomyManager economy)` | What the sector can SEE it will sell a month over the next `months`, when its customers' orders are on a book - a ceiling on the demand the maker's rule reads off the trend, since a trend read off a boom runs on after... |
| 1453 | 3 | `public double estimatedMonthlyProfit(BuildingsTemplate t, BusinessInvestment plans)` | What one of its templates would clear a month, for the interest test. |
| 1471 | 7 | `public double[] retirementDemandAndCapacity(Game game)` | The demand the spare-capacity rule measures the sector against, and the capacity it measures. |
| 1480 | 4 | `public double unitsOf(BuildingsTemplate t)` | What one of these contributes to the measure the sector is judged on. |
| 1486 | 1 | `public boolean mayRetire(BuildingsTemplate t)` | Whether a holding of this template may be sold back this month. |
| 1489 | 3 | `public String noRetirementReason(boolean distress)` | Why nothing could be sold, when mayRetire() refused everything. |

### THE SCREENS (lines 1493-1649)

| line | len | member | says |
|---:|---:|---|---|
| 1502 | 9 | **type** `public record Line(Kind kind, String label, String value, Tone tone)` | One line of the operations page. |
| 1503 | 1 | **type** `public enum Kind` _(in Sector.Line)_ |  |
| 1504 | 1 | **type** `public enum Tone` _(in Sector.Line)_ |  |
| 1506 | 1 | `public static Line head(String text)` _(in Sector.Line)_ |  |
| 1507 | 1 | `public static Line note(String text)` _(in Sector.Line)_ |  |
| 1508 | 1 | `public static Line of(String label, String value)` _(in Sector.Line)_ |  |
| 1509 | 1 | `public static Line of(String label, String value, Tone tone)` _(in Sector.Line)_ |  |
| 1513 | 9 | `public String inputLabel()` | What the sector's direct-cost line is called on its income statement. |
| 1539 | 1 | `public Map<String, Double> otherRevenueParts()` | The parts of this sector's revenue that are not the sale of a good, named, for the lines inside an opened Revenue. |
| 1549 | 1 | `public Map<String, Double> otherInputParts()` | ...and the same for the cost line: what is in Inputs that is not a good. |
| 1569 | 5 | `protected Map<String, Double> nameOtherRevenue()` | ...and where those names come from, read off the sector's LIVE fields at the moment the month is struck. |
| 1575 | 74 | `public List<Line> operations(Game game)` |  |

### SAVE AND RESTORE (lines 1650-1784)

| line | len | member | says |
|---:|---:|---|---|
| 1655 | 18 | `public SectorState toState()` | Everything about this sector that a save has to carry. |
| 1674 | 47 | `public void restore(SectorState s)` |  |
| 1723 | 7 | `public void restoreBills(SectorState s)` | The bills of the month back, over whatever a rebuild re-derived. |
| 1732 | 1 | `protected void saveExtras(Map<String, Double> extras)` | A sector with state of its own - a price it walks, an order book - writes it here by name. |
| 1735 | 1 | `protected void restoreExtras(Map<String, Double> extras)` | ...and reads it back. |
| 1738 | 18 | `public void reset()` | Everything back to a founding sector. |
| 1757 | 1 | `protected void resetExtras()` |  |
| 1763 | 16 | `public void redenominate(double scale)` | Its money in the new unit. |
| 1780 | 1 | `protected void redenominateExtras(double scale)` |  |
| 1783 | 1 | `public String toString()` |  |
