# WorldEconomy.java - 396 lines · 17 methods · 11 constants · model

`ham/citybuildersim/WorldEconomy.java` - generated 2026-09-24 by CodeMap; line numbers are as of that run.
...
| 322 | `private final double[] levels` | The level at the end of each of the last thirteen months; head is the newest. |
| 323 | `private int head` |  |

## Methods, in file order, under their sections

| line | len | member | says |
|---:|---:|---|---|
| 38 | 359 | **type** `public class WorldEconomy` | The rest of the world, which has its own inflation and did not use to. |
...
### WHY THE DRAW IS BIASED LOW, AND WHY THE BAND WAS A FICTION (lines 114-342)
...
| 314 | 5 | `public double realisedInflation()` | What the world's prices ACTUALLY did over the last twelve months, from the level itself. |
| 328 | 1 | `{ ... }` | A fresh world has a year of history at its opening rate, so the first twelve months read the headline rather than zero. |
| 330 | 4 | `private void recordLevel()` |  |
| 336 | 6 | `private void backcastLevels()` | Fills the ring as if the headline rate had held for a year. |

### carrying (lines 343-396)

| line | len | member | says |
|---:|---:|---|---|
| 345 | 14 | `public double[] toSaveArray()` |  |
| 360 | 23 | `public void restore(double[] saved)` |  |
| 387 | 9 | `public void reset()` |  |
