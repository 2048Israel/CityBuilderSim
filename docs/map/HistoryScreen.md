# HistoryScreen.java - 1,442 lines · 32 methods · 6 constants · interface
...
|---:|---:|---|---|
| 36 | 1407 | **type** `final class HistoryScreen` | The Reports tab: the city as a shape over time. |
...
| 237 | 9 | `static Trace[] withTheCrime(Trace[] fixed)` | One series per household shape, appended after the market. |
| 247 | 9 | `static Trace[] withTheHouseholds(Trace[] fixed)` |  |
| 257 | 9 | `static Trace[] withTheMarket(Trace[] fixed)` |  |
...
| 812 | 20 | `static String axisTick(String unit, double v, double step)` | One gridline's label: short enough to fit, honest about its unit. |
| 842 | 5 | `static String priceTick(double a, double step)` | A price on an axis: cents only when the gridlines are finer than a dollar. |
| 849 | 6 | `static String shortCash(double a)` | Dollars, abbreviated from a thousand up - an axis has no room for digits. |
| 857 | 5 | `static String shortCount(double a)` | People, homes, jobs - the same abbreviation without the dollar. |
| 864 | 4 | `static String trim(double a)` | One decimal at most, and none at all when it would read ".0". |
| 869 | 6 | `static double niceStep(double raw)` |  |
...
| 1145 | 14 | `static double[] trailingSum(double[] series, int window)` | A trailing total over the last `window` readings. |
| 1160 | 156 | `double[] historyValues(HistorySave h, String key)` |  |
...
### THE GOODS ROW (lines 1386-1442)

| line | len | member | says |
|---:|---:|---|---|
| 1402 | 40 | `HBox goodsRow(Good g)` | One good: what it is, what it costs here, and what happened to it. |
