...
**Used by (22):** [BusinessServicesCheck](BusinessServicesCheck.md), [CarCheck](CarCheck.md), [CityBasket](CityBasket.md), [EquityCheck](EquityCheck.md), [Exchange](Exchange.md), [ExchangeCheck](ExchangeCheck.md), [Game](Game.md), [HouseholdAccounts](HouseholdAccounts.md), [HouseholdBalance](HouseholdBalance.md), [HouseholdCheck](HouseholdCheck.md), [LongPlaytest](LongPlaytest.md), [Offending](Offending.md), [OrphanHousehold](OrphanHousehold.md), [OutsideCheck](OutsideCheck.md), [PeopleScreen](PeopleScreen.md), [PrisonerHousehold](PrisonerHousehold.md), [RestaurantsCheck](RestaurantsCheck.md), [RetiredHousehold](RetiredHousehold.md), [ShadowBasket](ShadowBasket.md), [StudentHousehold](StudentHousehold.md), [UnemployedHousehold](UnemployedHousehold.md), [WorkingHousehold](WorkingHousehold.md)
...
|---:|---|
| 98 | · the position |
| 250 | · last month's working, per household |
| 295 | · what differs |
| 354 | · reading |
...
| line | field | says |
|---:|---|---|
| 96 | `protected final FamilyStructure shape` | The family shape, or null for a household that is not one: the unemployed, a student, an orphan. |
| 104 | `double savings` | What one of these households has in the bank. |
| 107 | `double debt` | ...and what it owes the bank on its revolving credit. |
| 126 | `double investmentIncome` | Dividends and foreign coupons received since this cell last planned a month. |
| 129 | `int lockout` | Months this cell cannot borrow, after a discharge. |
| 136 | `double studentDebt` | What one of these households owes the treasury on student loans. |
| 148 | `final double[] shares` | Shares held in each of the city's companies, per household of the cell, indexed as Equity.COMPANIES. |
| 151 | `double dividends` | What the shares paid this month, per household. |
| 166 | `double abroad` | DOLLARS held abroad by one of these households: the world's paper, bought with savings past the cushion when the world pays more than the bank, sold when the bank pays more or the household needs the money. |
| 191 | `double cars` | CARS THIS HOUSEHOLD OWNS, per household of the cell, 0 to 1 (2026-09-16). |
| 197 | `double luxuryWant` | What this household would spend on luxuries this month, per household. |
| 203 | `double mealWant` | ...and what it would spend eating out, per household. |
| 228 | `double mealsEaten` | MEALS this household ate out last month, per household. |
| 238 | `double carsSold` | ...and the ones this cell sold into the second-hand market this month, in total rather than per household. |
| 241 | `double sentAbroad, broughtHome, foreignInterest` | This month's, per household, in local money: sent abroad, brought home, and earned there (rolled, not paid home). |
| 248 | `double households` | Households this cell was last struck for - the multiplier on every per-household figure, and the count the next month's census is compared against to see who moved. |
| 257 | `double disposable` | Take-home this month: wages or pension, after tax and contributions. |
| 258 | `double afterFixed` |  |
| 259 | `double interest` |  |
| 260 | `double drawn` |  |
| 261 | `double unfunded` |  |
| 262 | `double borrowed` |  |
...
| 266 | `double planned` |  |
| 267 | `double rate` |  |
| 268 | `double subsistence` |  |
| 271 | `double bankrupt` | Households of this cell discharged this month - a count, not money. |
| 274 | `double sold` | Shares sold this month to cover the shop, per household, in cash. |
| 277 | `double studentBorrowed, studentRepaid` | Student loan drawn this month, and repaid, per household. |
| 280 | `double evicted` | Households of this cell that lost their home this month - a count, not money. |
| 283 | `double rentShare` | What one of these households paid of a door's rent this month: 1 alone, a fifth sharing, 0 with no door. |
| 387 | `double dependants` | Dependants who live in one of these households but are not its shape. |
...
| line | len | member | says |
|---:|---:|---|---|
| 230 | 1 | `public double mealsEaten()` |  |

### last month's working, per household (lines 250-294)

| line | len | member | says |
|---:|---:|---|---|
| 286 | 4 | **type** `interface Liquidity` | Somewhere a household short of money can sell shares before it borrows. |
| 288 | 1 | `double sell(Household cell, double needPer)` _(in Household.Liquidity)_ |  |
| 291 | 3 | `protected Household(FamilyStructure shape)` |  |

### what differs (lines 295-353)

| line | len | member | says |
|---:|---:|---|---|
| 298 | 1 | `public abstract PayTier tier()` | The pay tier, or null for a household with no earner. |
| 301 | 1 | `public abstract int row()` | The row this cell sums into: the tier's index, or RETIRED_ROW. |
| 303 | 1 | `public abstract boolean isRetired()` |  |
| 315 | 1 | `public abstract int grownUps()` | Who in the household carries the money: the earners, or the pensioners. |
| 322 | 1 | `public double earningWeight()` | Who in the household the row's INCOME is split by. |
| 325 | 1 | `public boolean canBeEvicted()` | True for a cell whose households lose their home when they cannot pay for it. |
| 332 | 1 | `public int stockGroup()` | The row whose people this cell's people most often ARE, for the money to follow them: its own row, for everybody but the out of work. |
| 339 | 1 | `protected double baskets()` | Baskets of food one of these households has to buy: one a head, for everybody but a prisoner, whom the city feeds (the prisons' upkeep). |
| 345 | 1 | `protected boolean debtFrozen()` | True when the debt is frozen: no interest charged, nothing discharged, nothing borrowed. |
| 352 | 1 | `public boolean canInvest()` | True when the household decides what to do with its savings - shares, paper abroad, an offering. |

### reading (lines 354-360)
...
| line | len | member | says |
|---:|---:|---|---|
| 390 | 1 | `public double headcount()` | People in one of these households, counting anybody who came with them. |
| 393 | 1 | `public double getDependants()` | ...of whom this many live here without being its shape's own. |
| 395 | 4 | `public String label()` |  |
| 401 | 3 | `public String key()` | "COUPLE_TEEN:SKILLED", or the shape alone for the retired. |
| 405 | 1 | `public double households()` |  |
| 407 | 1 | `public double people()` | People in the whole cell - its households times what each of them holds. |
| 409 | 1 | `public double studentDebt()` |  |
| 410 | 1 | `public double studentBorrowed()` |  |
| 411 | 1 | `public double studentRepaid()` |  |
| 412 | 1 | `public double totalStudentDebt()` |  |
| 413 | 1 | `public double evicted()` |  |
| 414 | 1 | `public double rentShare()` |  |
| 417 | 1 | `public double savings()` | Per household. |
| 418 | 1 | `public double debt()` |  |
| 419 | 1 | `public int lockout()` |  |
| 422 | 1 | `public double shares(int company)` | Shares held in this company, per household. |
| 423 | 1 | `public double totalShares(int company)` |  |
| 426 | 1 | `public double dividends()` | This month's dividends, per household. |
| 429 | 1 | `public double abroad()` | Dollars held abroad, per household. |
| 430 | 1 | `public double cars()` |  |
| 432 | 1 | `public double totalCars()` | Every car this cell's households own between them. |
| 434 | 1 | `public double abroadValue(double localPerUsd)` | ...worth this much at home, per household, at a rate. |
| 435 | 1 | `public double sentAbroad()` |  |
| 436 | 1 | `public double broughtHome()` |  |
| 437 | 1 | `public double foreignInterest()` |  |
| 440 | 1 | `public double sold()` | Shares sold this month to cover the shop, per household, in cash. |
| 441 | 1 | `public boolean isLockedOut()` |  |
| 442 | 1 | `public double disposable()` |  |
| 443 | 1 | `public double afterFixed()` |  |
| 444 | 1 | `public double interest()` |  |
| 445 | 1 | `public double drawn()` |  |
| 446 | 1 | `public double unfunded()` |  |
| 447 | 1 | `public double borrowed()` |  |
| 448 | 1 | `public double repaid()` |  |
| 449 | 1 | `public double banked()` |  |
| 450 | 1 | `public double want()` |  |
| 451 | 1 | `public double planned()` |  |
| 452 | 1 | `public double rate()` |  |
| 453 | 1 | `public double subsistence()` |  |
| 456 | 1 | `public double luxuryWant()` | What this household would put over a luxury counter this month. |
