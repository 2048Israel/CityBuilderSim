# ReadPathCheck.java - 1,048 lines · 7 methods · 0 constants · harnesses
...
|---:|---:|---|---|
| 45 | 1004 | **type** `public class ReadPathCheck` | Reading the city must not change the city. |
| 51 | 4 | `static void assertTrue(String label, boolean ok)` |  |
| 56 | 6 | `static BuildingsTemplate template(Game game, String name)` |  |
| 68 | 123 | `static void bankPrint(Game g, Map<String, Double> into)` | The bank's own fields beside NewGameCheck's (0.7.7): its price is now struck from records it keeps, and a read that struck it again would move the price and nothing in the shared snapshot. |
| 193 | 5 | `static int restingOrders(BondMarket bm)` | What rests on every bond's book. |
| 207 | 545 | `static void readEverything(Game g)` | Everything a screen can ask the game, called the way a player browsing would call it. |
| 753 | 287 | `public static void main(String[] args) throws Exception` |  |
| 1041 | 7 | `static void cleanUp(Path root)` |  |
