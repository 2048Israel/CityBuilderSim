...
- `land-in-dollars.md` - 0.7.6 (2026-09-23): land priced in US dollars and paid at the day's rate, by converting cash (the default) or out of the vault, a toggle at the top of the land office; one `ui/Ladder` for every policy dial; a price per kind of school with a row per kind (places, students, cost, revenue); real GDP in layers, C, I and G stacked under the line with net exports the gap; and why converting for land puts no push on the currency - open, Jerus's call
- `financial-crisis-of-2045.md` - 0.7.5 (2026-09-23): Enter builds and Backspace clears on the build rail; the Reports page redrawn — two pinned small charts as a preference, two real axes before the 0–100 squash, a crosshair, recessions shaded, the named episodes (`YearBook.episodes()`, the "little name generator") ticked under the chart and in the year book, the picker folded with a filter, a log switch
- `every-tax-at-once.md` - the housekeeping of 0.7.4 (2026-09-23): the inflation target as a dial, the price of money three ways on the strip, the sector list's sparkline and "show more" on two new history series, and a base rate per income tax with the old city rate as "every tax at once"; the load-path trap that would have merged a split
...
The commercial bank, the central bank (built in 0.7.0; the curve, the holders and the holdings dial in 0.7.1; the currency under it in 0.7.2; the demand channel in 0.7.3; the bank as a business from 0.7.7), the float, capital flows and the audit that ties every pool.
...
- `the-rate-that-stops-the-cranes.md` - Jerus's 0.7.5 year book read against the code (2026-09-23): the autopilot on until about 2050, two inflations that were rent under a labour shortage, the housing stock standing still; eight seeds a setup from a 0% to a 25% dial; the mechanism — a landlord that will not build on credit past a few points, and rent floored on its own interest bill — a loop with a gain above one, which the strain premium deepened; what it means for the outside brief
- `the-bank-as-a-business-plan.md` - the research (Canada's big banks: prime at the dial plus 2.20, a deposit beta of ~0.4, losses ~0.4% of loans, CET1 11–13.5%, the window at the target plus a quarter point, IFRS 9's stages, TARP) and the plan in three batches — pricing and fees, capital and payout, the Bank tab — and the six questions put to Jerus, 2026-09-23; step 1 of the order agreed after the diagnosis (the bank, then housing against the rate, prices that answer spending, a smaller founding treasury, a negative floor only if needed)
- `the-bank-prices-like-a-business.md` - 0.7.7 (2026-09-23), batch 1 of the bank as a business: the strain premium gone and a loan priced from its four costs (the funds-transfer price, running costs, a through-the-cycle expected loss, a capital charge at the owners' 12.5%), prime a sound business's rate; savers paid a share of the dial the bank's funding sets, bounded by the margin after costs and by the savers' share of it; an account fee and a loan fee; the window at the dial plus a quarter point; dividends after tax; why the expected loss is a constant, the held-10% catch, and what the eight seeds and the two stress setups measured
- `a-sector-is-many-firms.md` - 0.7.8 (2026-09-23/24), batch 2 of the bank as a business and the four rounds it took: the loss allowance in two stages, the bank's own capital target capped at the Basel stack (`MAX_BUFFER`), a payout rule and lending that tightens with capital, its own shares booked as capital; 92 failures as first built because a whole sector was one borrower, and Jerus's option C — a sector defaults a slice at a time off Merton's curve (`ASSET_VOLATILITY` 0.25), the whole-sector restructure only the backstop, the allowance staged firm by firm, loans priced by the curve on the last quarter's statements, a failing sector's plant sold to the builders for its material, the buybacks and the desk held to spare capital; syndication tried and deleted; 39 failures, every one a lending loss on a sector holding 30–73% of the book
- `the-bank-tab.md` - 0.7.9 (2026-09-23), batch 3 of the bank as a business: the Bank tab rebuilt — a status sentence, a scorecard of eight and the ladder of its rates from the policy rate to every borrower, then Profit, Lending, Funding, Capital & owners and History; every figure a model getter (`Bank`'s WHAT THE BANK TAB READS), a year of the bank's statements saved, what the old tab worked out wrongly; nothing the bank decides moved
- `founding-a-city.md` - 0.7.10 (2026-09-24): the endowment cut to D$100M and US$25M (the city borrows for its big works; the vault at three months of a young town's imports); the Found a city screen (a name, the money named after the city or typed, four presets priced at invoices, the world's inflation off Settings); the currency a value per city; old saves as Danzik and DZD; the sticker is not the invoice; 42 fixtures that lived on the old treasury
- `the-landlords-take-a-mortgage.md` - 0.7.11 (2026-09-24), the housing-against-the-rate step: the landlords' 3-year bullets replaced by insured CMHC-style mortgages (15% down, 85% loan-to-cost, 1.20× coverage, 40-year amortization fixed for 10), the city as insurer (premiums to the treasury, claims to the bank, 0% risk weight); round 2's Basel leverage ratio, branches that close, and dividends after principal; the bank's failures traced to concentration, `MonetaryCheck`'s allowance re-measured, and the smaller default city traced to crime
- `the-firms-sell-bonds.md` - 0.7.12 (2026-09-25/26), corporate notes and bonds over eight rounds: a 10-year bond bookbuilt beside the bank loan and traded with shares on one order book (the dealer gone, the desk a participant with caps, buybacks as bids, no special dividend), concentration priced by Basel IRB, recoveries by instrument (75% loans, 45% bonds), nothing lent past the default point on the quarter; then how a firm that can't pay ends — the BIA cash-flow test, credit lines that stay open under rationing, interim financing ranked first, and "buy only what it can pay for"; Luxury Retail's swing a price nobody paid; failures 115 → 1 (default); ships with HealthCheck's hunger gap red
- `rolling-what-falls-due.md` - 0.7.13 (2026-09-26), housekeeping: the land office prices in the paying currency and km², buys short on a funding page (local or dollar paper) and N plots at once; new games on the autopilot; the treasury's rollover (by hand / same structure / 12-month notes, net of last year's surplus, sized to the cash - and the Lean seed that spirals, with nothing capping a city's paper at home); the Bank tab's Balance sheet page, a year ago beside, equity as paid-in and retained, deposits a memo
...
- `the-founding-endowment.md` - the cash a city is founded with, and why it is what it is (superseded for the figures by `founding-a-city.md`, 0.7.10)
